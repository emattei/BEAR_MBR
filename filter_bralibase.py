...
for f in files:
    with open(f, "r") as handle:
        for record in SeqIO.parse(handle, "fastq"):
            ss_cons[record.id.replace("/", "_")] = "".join([chr(c+33)
                                                            for c in record.letter_annotations["phred_quality"]])
...
for f in files:
    with open(f, "r") as handle:
        ...
        if set(tmp.keys()) < keys:
            ...
            #copy the reference file to the destination
            destination=outfile[:-7]+".ref.fa"
